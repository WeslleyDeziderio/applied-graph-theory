...
std::string Data::getInstanceName() { return instanceName; }
...
void Data::determineDegreeOpenClosedNeighbor() {
  ...
  std::cout << "\nInsert the vertex to verify open closed adjacency: "
            << std::endl;
  std::cin >> vertex;
...
void Data::isSubgraph() {
  ...
  for (std::list<int> edge : edgesList) {
    if (edge.size() != 2) {
      std::cout << "\nThe given sequence does not form a valid edge!"
                << std::endl;
      return;
    }

    if (std::find(verticesList.begin(), verticesList.end(), edge.front()) ==
            verticesList.end() ||
        std::find(verticesList.begin(), verticesList.end(), edge.back()) ==
            verticesList.end()) {
      std::cout << "\nThe given sequence contains vertices not present in the "
                   "vertex list!"
                << std::endl;
      return;
    }
...
void Data::isWalk() {
  ...
  std::cout << "\n\nInsert the lenght of the list of vertices to determine if "
               "forms a walk: ";
  ...
  for (int i = 0; i < walkListSize - 1; ++i) {
    for (int j = i + 1; j < i + 2; ++j) {
      if (walkSequence[i] == walkSequence[j] ||
          !isAdjacency(walkSequence[i], walkSequence[j])) {
        std::cout << "\n The given sequence does not foram a walk! \n";
      }
    }
  }
...
void Data::isTrail() {
  ...
  std::vector<std::vector<bool>> visitedEdge(
      this->adjacencyList.size(),
      std::vector<bool>(this->adjacencyList.size(), false));
  std::cout
  ...
  for (int i = 0; i < trailSize - 1; ++i) {
    for (int j = i + 1; j < i + 2; ++j) {
      if ((trailSequence[i] == trailSequence[j]) ||
          (!isAdjacency(trailSequence[i], trailSequence[j])) ||
          (visitedEdge[trailSequence[i]][trailSequence[j]])) {
        std::cout << "The sequence does not form a trail. \n";
      }
      visitedEdge[trailSequence[i]][trailSequence[j]] = true;
      visitedEdge[trailSequence[j]][trailSequence[i]] = true;
    }
  }
  ...
  for (int i = 0; i < pathSequence.size() - 1; ++i) {
    for (int j = i + 1; j < i + 2; ++j) {
      if (pathSequence[i] == pathSequence[j] ||
          !isAdjacency(pathSequence[i], pathSequence[j]) ||
          visitedVertex[pathSequence[j]]) {
        std::cout << "The sequence does not form a path! \n";
      }
    }
    visitedVertex[pathSequence[i]] = true;
  }

  std::cout << "The sequence form a path! \n";
}
...
bool Data::isClique() {
  ...
  std::cout << "\nInsert the length of the sequence of edges to verify if is "
               "clique: ";
  std::cin >> cliqueSize;
  ...
  for (int i = 0; i < cliqueSequence.size(); i++) {
    for (int j = i + 1; j < cliqueSequence.size(); j++) {
      if (!isAdjacency(cliqueSequence[i], cliqueSequence[j])) {
        std::cout << "\nThe given sequence does not form a clique!"
                  << std::endl;
        return false;
      }
    }
...
void Data::isCliqueMaximal() {
  ...
  for (int count = 0; count <= this->numVertices; ++count) {
    if (!(std::find(cliqueSequence.begin(), cliqueSequence.end(), count) !=
          cliqueSequence.end())) {
      ...
      for (int i = 0; i < cliqueSequence.size(); i++) {
        ...
        if (i == cliqueSequence.size() - 1) {
          std::cout << "\nThe given sequence does not form a maximum clique!"
                    << std::endl;
          return;
        }
      }
...
void Data::generateComplement() {
  ...
  std::cout << "\nComplement:" << std::endl;
  this->adjacencyComplementMatrix.assign(numVertices,
...
void Data::isIndependentSet() {
  ...
  std::cout << "\nThe given sequence does not form an independent set!"
            << std::endl;
}
